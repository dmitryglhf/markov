#!/usr/bin/env bash
# Tiny hook handler that loudly announces every event markov fires at it.
#
# markov pipes the event payload as JSON on stdin and sets PLUGIN_ROOT in the
...
# Both stderr (so it shows up alongside markov tracing) and a log file.
